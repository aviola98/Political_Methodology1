...
A diferença de voto entre o eleitor residente no estrangeiro e no Brasil pode estar relacionada ao fato de que muitas vezes o eleitor imigrante tende a votar na oposição, uma vez que pode ter deixado o país durante um período de dificuldades econômicas associada ao incumbente, o que está relacionado, de certa forma, com a Teoria do Voto Econômico, uma vez que a mesma postula que o eleitor tende a premiar ou não o incumbente tendo em base a performace econômica do mesmo.
...
No tocante à amostra, tem-se a seleção das eleições de 2002 à 2018 pelo fato de que os bancos correspondentes à essas eleições apresentarem os dados do segundo para as eleições ocorridas no exterior.
...
3.  **Análise das Estatísticas Descritivas**
...
ADICIOONAR VOTO NULO?


Para seguir na análise, foram calculadas as seguintes estatísticas descritivas a partir da porcentagem de votos por categoria e local de voto: a média, a mediana e os desvios padrões. Ou seja, duas medidas de tendência central (*i.e.*, mostram como é uma observação típica) e uma medida de dispersão (*i.e.*, mostra o quanto os dados estão dispersos em torno do centro). Os resultados estão apresentados na tabela abaixo: 
...
Apesar de que ao analisar o histograma se tenha chegado a conclusão de que o uso da mediana acabaria sendo uma medida mais apropriada no momento de se comparar essas estatísticas porque a média acaba sendo de certa forma influenciada pelo valor dos *outliers*, ao analisar a tabela nota-se que os valores da média e da mediana acabam sendo muito próximo de si (com exceção da média e mediana de votos no incumbente no exterior, que acabam tendo uma distância de relativamente alta, sendo 33.8 e 38.1 respectivamente). 
...
Ao olhar as médias de votos no incumbente e na oposição para os votos de brasileiros residentes no exterior, tem-se uma diferença notável em favor da oposição, tendo uma média de 60.72 em comparação com o incumbente, que possui uma média de 33.86. A tendência em favor da oposição para eleitores brasileiros no exterior se confirma ao analisarmos a mediana. A diferença entre medianas tem uma magnitude muito próxima à diferença entre as duas médias, tendo uma leve diminuida, sendo a mediana para os votos na oposição de valor 57.6 e a mediana para votos no incumbente 38.1.

Devido a essa variabilidade dos dados, como dito na análise do Histograma, tem-se que o uso da mediana acaba sendo uma medida mais apropriada no momento de se comparar essas estatísticas porque a média acaba sendo de certa forma influenciada pelo valor dos *outliers*.

Contudo, tem-se um problema ao fazer somente esta análise. O $X_1$ e o $X_2$ não são exôgenos, logo, não se pode concluir um teste e não se pode concluir que são eles que provocam o efeito em $Y$ ou se é outra coisa. Para contornar esse impasse, foi realizado um Teste de Hipótesis, que será apresentado nas próximas sessões.
...
$$
H_0: {\text{não existe diferença entre votos brasileiros no exterior e votos brasileiros no Brasil}} 
$$

$$
H_1: {\text{existe diferença entre votos brasileiros no exterior e votos brasileiros no Brasil}} 
$$
...
Os resultados apresentados nesse Trabalho Final apontam na direção de que se tem um 